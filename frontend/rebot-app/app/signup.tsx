...
export default function SignupScreen() {
  return <Screen><AppHeader title="가입하기" back /><View style={styles.body}><FormField label="이름" icon="person" placeholder="이름을 입력해주세요" /><FormField label="이메일" icon="mail" placeholder="이메일을 입력해주세요" keyboardType="email-address" /><FormField label="비밀번호" icon="lock-closed" placeholder="비밀번호를 입력해주세요" secureTextEntry /><PrimaryButton label="회원가입" onPress={() => router.push('/onboarding/basic')} style={styles.button} /><View style={styles.login}><Text style={styles.muted}>이미 계정이 있으신가요? </Text><Text style={styles.link} onPress={() => router.replace('/login')}>로그인</Text></View></View></Screen>;
}
