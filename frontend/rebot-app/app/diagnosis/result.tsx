...
export default function ResultScreen() {
  ...
  return <Screen scroll><AppHeader title="AI 자가진단" back /><View style={styles.body}>
    ...
    {solutions.map((solution, index) => <View key={`${solution.action}-${index}`} style={styles.recommendation}><View style={styles.row}><Ionicons name="checkmark-circle" size={26} color="#8A6B00" /><Text style={styles.action}>{solution.action}</Text></View><Text style={styles.reason}>{solution.reason}</Text>{solution.duration_minutes && <Text style={styles.meta}>예상 소요 시간 · {solution.duration_minutes}분</Text>}</View>)}
