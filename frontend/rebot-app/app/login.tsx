  ...
  return <Screen contentStyle={styles.screen}>
    <View style={styles.hero}>
      <View style={styles.logoRow}>
        <Text style={styles.logoText}>Re:Bot</Text>
        <View style={styles.logoDot} />
      </View>
    </View>
    <View style={styles.form}>
      <View style={[styles.line, emailFocused && styles.lineFocused]}><TextInput style={styles.input} value={email} onChangeText={setEmail} onFocus={() => setEmailFocused(true)} onBlur={() => setEmailFocused(false)} placeholder="이메일을 입력하세요" autoCapitalize="none" autoCorrect={false} keyboardType="email-address" /><Pressable hitSlop={8} onPress={() => setEmail('')}><Ionicons name="close-circle" size={19} color="#8D8D96" /></Pressable></View>
      <View style={[styles.line, passwordFocused && styles.lineFocused]}><TextInput style={styles.input} value={password} onChangeText={setPassword} onFocus={() => setPasswordFocused(true)} onBlur={() => setPasswordFocused(false)} placeholder="비밀번호를 입력하세요" placeholderTextColor="#9CA4AB" secureTextEntry={!passwordVisible} onSubmitEditing={handleLogin} /><Pressable hitSlop={8} onPress={() => setPasswordVisible((current) => !current)}><Ionicons name={passwordVisible ? 'eye-outline' : 'eye-off-outline'} size={20} color="#9CA4AB" /></Pressable></View>
...
const styles = StyleSheet.create({ screen: { paddingHorizontal: 24 }, hero: { marginTop: 64, marginBottom: 48, alignItems: 'center' }, logoRow: { flexDirection: 'row', alignItems: 'flex-start' }, logoText: { fontSize: 34, fontWeight: '900', letterSpacing: 0.5, color: colors.text }, logoDot: { width: 7, height: 7, borderRadius: 4, marginTop: 6, marginLeft: 3, backgroundColor: colors.accent }, form: { gap: 0 }, line: { height: 70, borderBottomWidth: 1, borderBottomColor: colors.border, flexDirection: 'row', alignItems: 'center' }, lineFocused: { borderBottomColor: colors.black }, input: { flex: 1, fontSize: 15 }, options: { height: 70, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, option: { fontSize: 13, color: colors.subtle }, forgot: { fontSize: 13, color: colors.subtle, textDecorationLine: 'underline' }, kakao: { height: 60, marginTop: 12, borderRadius: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, backgroundColor: '#FEE500' }, kakaoPressed: { opacity: 0.72 }, kakaoDisabled: { opacity: 0.55 }, kakaoLabel: { fontSize: 17, fontWeight: '700', color: '#191919' }, bottom: { marginTop: 'auto', marginBottom: 34, paddingTop: 28, flexDirection: 'row', justifyContent: 'center' }, bottomText: { fontSize: 14, color: colors.muted }, signup: { fontSize: 14, color: '#C39B00', fontWeight: '700', textDecorationLine: 'underline' } });
