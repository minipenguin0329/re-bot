...
export function ChoiceCard({ title, description, selected, onPress, number }: Props) {
  ...
  return <Pressable onPress={onPress} style={[styles.card, selected && styles.selected]}>{number ? <Text style={styles.number}>{number}</Text> : null}<View style={styles.copy}><Text style={styles.title}>{title}</Text>{description ? <Text style={styles.description} numberOfLines={expanded ? undefined : 2} onPress={() => setExpanded((current) => !current)}>{description}</Text> : null}</View></Pressable>;
}
